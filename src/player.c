...
static int __msg_callback(int message, void *param, void *user_data)
{
	...
	switch(message)
	{
		...
		case MM_MESSAGE_DRM_NOT_AUTHORIZED: //0x111
			...
			err_code = PLAYER_ERROR_NOT_SUPPORTED_FILE;
			break;
                case MM_MESSAGE_DRM_NO_LICENSE: //0x117
                        LOGE("[%s] PLAYER_ERROR_DRM_NO_LICENSE (0x%08x) : DRM_NO_LICENSE" ,__FUNCTION__, PLAYER_ERROR_DRM_NO_LICENSE);
                        err_code = PLAYER_ERROR_DRM_NO_LICENSE;
                        break;
                case MM_MESSAGE_DRM_EXPIRED: //0x118
                        LOGE("[%s] PLAYER_ERROR_DRM_EXPIRED (0x%08x) : DRM_EXPIRED" ,__FUNCTION__, PLAYER_ERROR_DRM_EXPIRED);
                        err_code = PLAYER_ERROR_DRM_EXPIRED;
                        break;
                case MM_MESSAGE_DRM_FUTURE_USE: //0x119
                        LOGE("[%s] PLAYER_ERROR_DRM_FUTURE_USE (0x%08x) : DRM_FUTURE_USE" ,__FUNCTION__, PLAYER_ERROR_DRM_FUTURE_USE);
                        err_code = PLAYER_ERROR_DRM_FUTURE_USE;
                        break;
