	...
	if (handle->user_cb[_PLAYER_EVENT_TYPE_MEDIA_PACKET_VIDEO_FRAME]) {
		...
		if (tsurf) {
			...
			if (handle->pkt_fmt) {
				...
				if (pkt_fmt_mimetype != mimetype || pkt_fmt_width != video_stream->width || pkt_fmt_height != video_stream->height) {
					LOGW("different format. current 0x%x, %dx%d, new 0x%x, %dx%d", pkt_fmt_mimetype, pkt_fmt_width, pkt_fmt_height, mimetype, video_stream->width, video_stream->height);
