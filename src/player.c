...
int client_wait_for_cb_return(muse_player_api_e api, callback_cb_info_s * cb_info, char **ret_buf, int time_out)
{
	...
	if (!buff->recved || !msg) {
		if (!g_cond_wait_until(&cb_info->player_cond[api], &cb_info->player_mutex, end_time)) {
			...
			g_mutex_unlock(&cb_info->player_mutex);
			return PLAYER_ERROR_SERVICE_NO_RESPONSE;
		}
	}
	if (!msg)
		msg = _get_ret_msg(api, cb_info);
...
int client_wait_for_server_ack(muse_player_api_e api, callback_cb_info_s * cb_info, int time_out)
{
	...
	if (!buff->recved) {
		if (!g_cond_wait_until(&cb_info->server_ack_cond, &cb_info->player_mutex, end_time)) {
			...
			if (!buff->recved)
				ret =  PLAYER_ERROR_SERVICE_NO_RESPONSE;
			else
...
int player_create(player_h * player)
{
	...
	while (--Nretry) {
		...
		if (ret == PLAYER_ERROR_SERVICE_NO_RESPONSE) {
			retry = true;
			goto ERROR;
		}
		retry = false;
 ...
 ERROR:
		...
		g_free(ret_buf);
		ret_buf = NULL;
		LOGE("ret value : %d, retry #%d", ret, CONNECTION_RETRY - Nretry);
		if (!retry)
			break;
	}
	g_free(pc);
